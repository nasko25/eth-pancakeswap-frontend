 ...
 */
contract RBRYSwapLottery is ReentrancyGuard, ILottery, Ownable {
    ...
    uint256 public maxPriceTicketInRBRY = 50 ether;
    uint256 public minPriceTicketInRBRY = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInRBRY;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] RBRYPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInRBRY;
        uint32 finalNumber;
    }
     ...
     * @param _RBRYTokenAddress: address of the RBRY token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _RBRYTokenAddress, address _randomGeneratorAddress) {
        RBRYToken = IERC20(_RBRYTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of RBRY to this contract
        uint256 amountRBRYToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInRBRY,
            _ticketNumbers.length
        );

        // Transfer RBRY tokens to this contract
        RBRYToken.safeTransferFrom(address(msg.sender), address(this), amountRBRYToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInRBRY += amountRBRYToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInRBRYToTransfer
        uint256 rewardInRBRYToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInRBRYToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        RBRYToken.safeTransfer(msg.sender, rewardInRBRYToTransfer);

        emit TicketsClaim(msg.sender, rewardInRBRYToTransfer, _lotteryId, _ticketIds.length);
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in RBRY per group, and make lottery claimable
     * @param _lotteryId: lottery id
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInRBRY) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in RBRY for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].RBRYPerBracket[j] =
                ...
                // A. No RBRY to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].RBRYPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInRBRY - amountToShareToWinners);

        // Transfer RBRY to treasury address
        RBRYToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in RBRY token
     * @dev Callable by owner or injector address
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        RBRYToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInRBRY += _amount;
     ...
     * @param _priceTicketInRBRY: price of a ticket in RBRY
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInRBRY >= minPriceTicketInRBRY) && (_priceTicketInRBRY <= maxPriceTicketInRBRY),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInRBRY: _priceTicketInRBRY,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            RBRYPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInRBRY: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(RBRYToken), "Cannot be RBRY token");
    ...
    /**
     * @notice Set RBRY price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInRBRY: minimum price of a ticket in RBRY
     * @param _maxPriceTicketInRBRY: maximum price of a ticket in RBRY
     */
    function setMinAndMaxTicketPriceInRBRY(uint256 _minPriceTicketInRBRY, uint256 _maxPriceTicketInRBRY)
        external
        onlyOwner
    {
        require(_minPriceTicketInRBRY <= _maxPriceTicketInRBRY, "minPrice must be < maxPrice");

        minPriceTicketInRBRY = _minPriceTicketInRBRY;
        maxPriceTicketInRBRY = _maxPriceTicketInRBRY;
    }
     ...
     * @param _priceTicket price of a ticket (in RBRY)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].RBRYPerBracket[_bracket];
        } else {
            return 0;
        }
    }
