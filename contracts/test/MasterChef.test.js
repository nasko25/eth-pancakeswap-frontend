...
contract('MasterChef', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async function() {
        ...
        this.chef = await MasterChef.new(this.raspberry.address, this.syrup.address, dev, '1000', '100', { from: minter });
        await this.raspberry.transferOwnership(this.chef.address, { from: minter });
    ...
    it('real case', async function() {
      ...
      assert.equal((await this.raspberry.balanceOf(alice)).toString(), '0');
      await this.chef.deposit(1, '20', { from: alice });
      await this.chef.withdraw(1, '20', { from: alice });
      assert.equal((await this.raspberry.balanceOf(alice)).toString(), '263');

      await this.raspberry.approve(this.chef.address, '1000', { from: alice });
      await this.chef.enterStaking('20', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      assert.equal((await this.raspberry.balanceOf(alice)).toString(), '993');
    ...
    it('deposit/withdraw', async function() {
      ...
      assert.equal((await this.raspberry.balanceOf(alice)).toString(), '999');
      assert.equal((await this.raspberry.balanceOf(dev)).toString(), '100');
      ...
      assert.equal((await this.raspberry.balanceOf(bob)).toString(), '125');
    ...
    it('updaate multiplier', async function() {
      ...
      await this.raspberry.approve(this.chef.address, '100', { from: alice });
      await this.raspberry.approve(this.chef.address, '100', { from: bob });
      ...
      assert.equal((await this.raspberry.balanceOf(alice)).toString(), '700');
      assert.equal((await this.raspberry.balanceOf(bob)).toString(), '150');
